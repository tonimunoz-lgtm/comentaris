...
async function carregarDadesButlletins(grups) {
  ...
  try {
    ...
    // Grup de tutoria associat (per al comentari del tutor al butlletí)
    const grupTutoria = totsGrups.find(g =>
      g.parentGrupId === grupId && g.tipus === 'tutoria'
    ) || null;
    ...
    // ── Llegir comentari de tutoria per cada alumne ──
    if (grupTutoria) {
      try {
        let snapTut = await db.collection('avaluacio_centre')
          .doc(curs).collection(grupTutoria.id)
          .where('grupClasseId', '==', grupId).get();
        if (snapTut.empty) {
          snapTut = await db.collection('avaluacio_centre')
            .doc(curs).collection(grupTutoria.id)
            .where('grupId', '==', grupId).get();
        }
        const docsTut = trimestre
          ? snapTut.docs.filter(d => d.data().periodeNom === trimestre)
          : snapTut.docs;
        docsTut.forEach(doc => {
          const d = doc.data();
          const key = d.ralc || `${d.cognoms}_${d.nom}`;
          if (alumnesAmbDades[key]) {
            alumnesAmbDades[key].comentariTutoria = d.comentariGlobal || '';
          }
        });
      } catch(e) { /* tutoria sense dades */ }
    }

    // ── Llegir PI per cada alumne ──
    try {
      const pisSnap = await db.collection('plans_individualitzats').where('actiu','==',true).get();
      pisSnap.docs.forEach(doc => {
        const ralc = doc.id;
        const piData = doc.data();
        const key = Object.keys(alumnesAmbDades).find(k => alumnesAmbDades[k].ralc === ralc);
        if (key) alumnesAmbDades[key].pi = { tipus: piData.tipus || '', adaptacio: piData.adaptacio || '' };
      });
    } catch(e) { /* PI no disponible */ }
...
async function generarButlleti(alumne, curs, grupNom, trimestre, grupId, nivellNom = '', infoButlleti = '') {
  ...
  const comentariTutoria = alumne.comentariTutoria || '';
  const pi = alumne.pi || null;
...
<body>
  ...
  <div style="margin-bottom:16px;border:1px solid #e5e7eb;border-radius:6px;overflow:hidden;">
    <div style="background:#1e1b4b;color:#fff;padding:7px 12px;font-weight:bold;font-size:11pt;-webkit-print-color-adjust:exact;print-color-adjust:exact;">Comentari Tutor/a</div>
    <div style="padding:10px 12px;font-size:10pt;line-height:1.6;white-space:pre-wrap;">${esH(comentariTutoria)}</div>
  </div>` : ''}

  <div style="margin-bottom:16px;border:1px solid #e5e7eb;border-radius:6px;overflow:hidden;">
    <div style="background:#374151;color:#fff;padding:7px 12px;font-weight:bold;font-size:11pt;-webkit-print-color-adjust:exact;print-color-adjust:exact;">Aspectes a tenir en compte</div>
    <table style="border:none;">
      <tr>
        <td style="width:220px;font-weight:600;font-size:10pt;border-bottom:1px solid #e5e7eb;padding:8px 12px;">ORIENTACIONS (si escau):</td>
        <td style="border-bottom:1px solid #e5e7eb;padding:8px 12px;font-size:10pt;"></td>
      </tr>
      <tr>
        <td style="font-weight:600;font-size:10pt;padding:8px 12px;">SUPORT EDUCATIU:</td>
        <td style="padding:8px 12px;font-size:10pt;">${pi ? 'Pla de Suport Individualitzat' : ''}</td>
      </tr>
    </table>
  </div>
  ...
  <div class="peu">
    ...
    <div class="firma-box">
      <div class="firma-img-area" style="position:relative;width:130px;height:60px;">
        ...
        ${segellBase64 ? `<img src="${segellBase64}" style="position:absolute;bottom:-6px;right:20px;max-height:62px;max-width:70px;object-fit:contain;opacity:0.88;">` : ''}
      </div>
