...
async function carregarDadesButlletins(grups) {
  ...
  try {
    ...
    document.getElementById('bGenTots')?.addEventListener('click', () => {
      ambDades.forEach(a => generarButlleti(a, curs, grupDoc?.nom||'', trimestre, grupId));
    });
    ...
    document.querySelectorAll('.btn-gen-butlleti').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (alumne) generarButlleti(alumne, curs, grupDoc?.nom||'', trimestre, grupId);
      });
    });
...
async function generarButlleti(alumne, curs, grupNom, trimestre, grupId) {
